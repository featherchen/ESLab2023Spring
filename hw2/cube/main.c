...
uint8_t RemoteIP[] = {192,168,50,145};
...
//    }
	  ...
	  len = sprintf(buf,"{\"x\":%d,\"y\":%d,\"z\":%d,\"s\":%d}",pDataXYZ[0],
	              pDataXYZ[1], pDataXYZ[2], sample_num);

	  ret = WIFI_SendData(Socket, buf, strlen(buf), &Datalen, WIFI_WRITE_TIMEOUT);
	  ...
	  len = sprintf(buf,"{\"x\":%d,\"y\":%d,\"z\":%d,\"s\":%d}",(int)(pfData[0] * ratio),
	  	              (int)(pfData[1] * ratio), (int)(pfData[2] * ratio), sample_num);
