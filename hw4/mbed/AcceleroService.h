...
public:
     ...
     */
    AcceleroService(BLE &_ble, int16_t arr[], BodySensorLocation location) :
        ble(_ble),
        valueBytes(arr),
        hrmRate(
     ...
     * BLE stack.
     */
    void updateaccelero(int16_t *arr) {
        valueBytes.updateHeartRate(arr);
...
protected:
    ...
    struct HeartRateValueBytes {
        ...
        HeartRateValueBytes(int16_t *arr) : valueBytes()
        {
            updateHeartRate(arr);
        }

        void updateHeartRate(int16_t *arr)
        {
            printf("---------------------\n");
            for (int i = 0; i < 3; i++) {
                printf("%d ", arr[i]);
                valueBytes[2 * i] = (uint8_t)(arr[i] & 0xFF);
                valueBytes[2 * i + 1] = (uint8_t)(arr[i] >> 8);
            }
            printf("\n");
            for (int i = 0; i < MAX_VALUE_BYTES; i++) {
                printf("%d ", (char)valueBytes[i]);
            }
            printf("\n");
        }
